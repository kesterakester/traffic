...
export default function WeeklyTrafficHeatmap() {
  ...
  const fetchWeeklyTrafficData = async () => {
    // Demo mode: Generate mock heatmap data
    setLoading(true)
    ...
    for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
      ...
      for (let hour = 0; hour < 24; hour++) {
        ...
        // Generate realistic traffic using helper function
        const congestion = generateRealisticTraffic(hour, dayName, isPrediction)
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Traffic Status: <span className={`font-medium ${selectedCell.level === 'severe' ? 'text-red-400' :
